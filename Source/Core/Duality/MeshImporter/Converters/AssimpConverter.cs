    ...
    class AssimpConverter : IMeshImporter
    {
        public Mesh Import(Stream stream)
        {
            using (var importer = new AssimpContext())
            {
                ...
                var model = importer.ImportFileFromStream(stream, PostProcessSteps.CalculateTangentSpace | PostProcessSteps.LimitBoneWeights | PostProcessSteps.FlipUVs);
                ...
                foreach (var meshToImport in model.Meshes)
                {
                    ...
                    {
                        Logs.Core.Write($"Invalid prrimitive type, should be triangle. {meshToImport.Name}");
                        continue;
                    }

                    if (!meshToImport.HasNormals)
                    {
						Logs.Core.Write($"Does not have any normals. {meshToImport.Name}");
                        continue;
                    }

                    if (!meshToImport.HasTangentBasis)
                    {
						Logs.Core.Write($"Does not have any tangents. {meshToImport.Name}");
                        continue;
                    }

                    if (meshToImport.TextureCoordinateChannelCount == 0)
                    {
						Logs.Core.Write($"Does not have any texture channels. {meshToImport.Name}");
                        continue;
                    }
